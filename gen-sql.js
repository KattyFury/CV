...
const vals = data.map(e =>
  `('${e.ticker}','${e.tgeDate}',${e.fundraising},${e.vcAlloc},${e.totalSupply},${e.priceTGE})`
).join(',\n');

const sql = `-- 1. Tạo bảng
create table if not exists tge_entries (
  id           bigserial primary key,
  ticker       text    not null,
  tge_date     date    not null,
  fundraising  numeric not null,
  vc_alloc     numeric not null,
  total_supply numeric not null,
  price_tge    numeric not null,
  created_at   timestamptz default now(),
  unique(ticker, tge_date)
);

-- 2. Bật RLS + cấp quyền anon đọc/ghi (admin gate ở client-side)
alter table tge_entries enable row level security;

create policy allow_select on tge_entries for select using (true);
create policy allow_insert on tge_entries for insert with check (true);
create policy allow_update on tge_entries for update using (true) with check (true);
create policy allow_delete on tge_entries for delete using (true);

-- 3. Seed 51 dự án
insert into tge_entries (ticker, tge_date, fundraising, vc_alloc, total_supply, price_tge) values
${vals}
on conflict (ticker, tge_date) do nothing;
`;

fs.writeFileSync('C:/Users/p/Desktop/supabase-setup.sql', sql);
console.log('Done. File: C:/Users/p/Desktop/supabase-setup.sql');
